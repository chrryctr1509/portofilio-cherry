...
<template>
  <section class="py-20 lg:py-28 bg-gradient-to-b from-slate-50 to-white dark:from-slate-900 dark:to-slate-800" id="references">
    <div class="mx-auto container px-4 sm:px-6 lg:px-8">
      ...
      <header class="text-center max-w-3xl mx-auto mb-16">
        ...
        <p class="text-lg text-slate-600 dark:text-slate-300">
          Feedback from managers, colleagues, and collaborators
        </p>
      </header>
      ...
      <div class="max-w-6xl mx-auto">
        ...
        <div class="relative">
          <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl border-2 border-slate-200 dark:border-slate-700 overflow-hidden">
            <div class="grid lg:grid-cols-5 gap-8 p-8 lg:p-12">
              ...
              <div class="lg:col-span-2 flex flex-col items-center lg:items-start text-center lg:text-left">
                <div class="relative mb-6">
                  ...
                  <!-- Quote Icon -->
                  <div class="absolute -bottom-3 -right-3 h-14 w-14 rounded-full bg-slate-900 dark:bg-slate-700 flex items-center justify-center shadow-lg">
                ...
                <h3 class="text-2xl font-bold text-slate-900 dark:text-white mb-1">{{ current.name }}</h3>
                <p class="text-brand-600 dark:text-brand-400 font-semibold mb-1">{{ current.role }}</p>
                <p class="text-slate-500 dark:text-slate-400 text-sm">{{ current.company }}</p>
              </div>

              <!-- Right Side - Testimonial Text -->
              <div class="lg:col-span-3 flex flex-col justify-center">
                <p class="text-xl lg:text-2xl text-slate-700 dark:text-slate-300 leading-relaxed italic">
                  "{{ current.text }}"
                </p>
              </div>
            </div>
          ...
          <div class="flex items-center justify-center gap-4 mt-8">
            <button 
              @click="prev"
              class="h-12 w-12 rounded-full bg-white dark:bg-slate-800 border-2 border-slate-300 dark:border-slate-600 hover:border-brand-500 dark:hover:border-brand-400 hover:bg-brand-50 dark:hover:bg-slate-700 transition-all flex items-center justify-center group shadow-lg"
            >
              <svg class="w-6 h-6 text-slate-600 dark:text-slate-300 group-hover:text-brand-600 dark:group-hover:text-brand-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
